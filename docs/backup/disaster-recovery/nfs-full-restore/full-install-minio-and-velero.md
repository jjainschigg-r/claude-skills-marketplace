...
1. Deploy MinIO Operator in its own namespace:
    ...
    helm repo up
    helm install --namespace minio-operator --create-namespace operator \
      minio-operator/operator
    ```

2. Download the MinIO Tenant `values.yml` file, configure it, and install it
   in its own namespace:
    ...
    1. Configure a username and password for Minio:
        ...
        ```yaml
           [..]
             pools:
               ...
               # For standalone mode, supply 1. For distributed mode, supply 4
               # or more.
               ...
               - servers: 1
                 ...
                 name: pool-0
                 ###
                 # The number of volumes attached per MinIO Tenant Pod /
                 # Server.
                 volumesPerServer: 4
                 ###
    ...
    2. Install Velero:
        ...
        2. For the file system backup:
            ...
            !!! note
                ...
                Both the Source and Target clusters must configure Velero to
                point to the same MinIO Tenant URL and Backup Storage Location
                (BSL) bucket.
